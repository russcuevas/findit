...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['docId'], $_POST['action'])) {
    ...
    if ($action === 'approve') {
        // Firestore PATCH request (update status to approved)
        $updateUrl = "https://firestore.googleapis.com/v1/projects/findit-96080/databases/(default)/documents/items/$docId?key=YOUR_API_KEY";
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $updateUrl . "&updateMask.fieldPaths=status");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        if ($httpCode === 200) {
            ...
            $_SESSION['error'] = "Failed to approve item. Response: " . $response;
        }

    } elseif ($action === 'deny') {
        // Firestore DELETE request (remove document)
        $deleteUrl = "https://firestore.googleapis.com/v1/projects/findit-96080/databases/(default)/documents/items/$docId?key=YOUR_API_KEY";
        ...
        if ($httpCode === 200) {
            ...
            $_SESSION['error'] = "Failed to delete item. Response: " . $response;
        }
    }
...
<body class="text-white overflow-hidden" style="background-color: black;">
    <div class="flex h-screen">
        ...
        <div class="flex-1 p-6 overflow-y-auto">
            <h1 style="font-size: 30px !important; color: white">Welcome! <?= htmlspecialchars($barangayName) ?></h1>
